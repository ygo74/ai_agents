...
The authorisation server sends the person back to a URL this process controls.
Listening on ``127.0.0.1`` keeps the authorisation code on the machine: it never
crosses a network and never passes through a third party.

The listener serves exactly one redirect and stops.
"""
...
from mail_mcp.protocol.errors import UnavailableError
...
class LoopbackConsent:
    ...
    def _serve_once(self) -> tuple[str, str | None]:
        ...
        if "code" not in captured:
            raise UnavailableError(f"authorisation failed: {captured.get('error', 'no code was returned')}")
