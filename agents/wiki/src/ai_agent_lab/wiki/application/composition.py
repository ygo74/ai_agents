...
class WikiAgentCompositionRoot:
    ...
    def build(self, *, session_id: str) -> WikiAgentRuntime:
        ...
        _logger.info("Calling WikiToolsProvider.capabilities() to discover served tools...")
        capabilities = provider.capabilities(base_path=self._base_path)
        _logger.info("Served capabilities count: %d", len(capabilities))

        _logger.info("Calling WikiAgentCompositionRoot._registry() to build skill registry...")
        registry = self._registry(manifest, skills, capabilities)

        _logger.info("Calling LangGraphApprovalTranslator.interrupt_on() for human-in-the-loop policies...")
        approvals = LangGraphApprovalTranslator()
